...
# Figure 1B + Extended Data Figure 1C
## Load data frame
```{r}

df_fig1bc_f <- "data/Fig1B_EDFig1ABC/05_Fig1B_EDFig1ABC_Stripped_CombinedFeatures.csv"
...
# CD3
df_cd3 <- df_fig1bc %>%
  dplyr::select(source, CD3con250ms, CD3con250msBL, CD3tsa250ms, CD3tsa250msBL) %>%
  tidyr::pivot_longer(
    cols = c(CD3con250ms, CD3con250msBL, CD3tsa250ms, CD3tsa250msBL),
    names_to = "Marker",
...
```{r}
# --- N per violin: Fig 1B CD3 ---
# Groups: y = concentration level, condition = Signal/Blank
cat("Fig 1B CD3 - N per violin:\n")
df_cd3 %>% dplyr::count(y, condition) %>% print()

# --- N per violin: Fig 1B IBA1 ---
cat("\nFig 1B IBA1 - N per violin:\n")
df_iba1 %>% dplyr::count(y, condition) %>% print()

# --- N per violin: Extended Data Fig 1C ---
# Groups: y = marker, method = PASTA/Unamplified
cat("\nFig 1C - N per violin:\n")
df_edfig1c %>% dplyr::count(y, method) %>% print()
```
...
```{r}
## Extended Data Fig. 1C
edfig_1c <- plot_violin_comparison(
  df_edfig1c,
  aes_fill = "method",
  ...
  base_size = 6,
  dodge_width = 0.8,
  filename = "output/ExtendedDataForFig1/ExtendedDataFig1C.pdf",
  width_pt = 190,
  height_pt = 160
)
edfig_1c
...
df_stripped_f <- "data/Fig1B_EDFig1ABC/05_Fig1B_EDFig1ABC_Stripped_CombinedFeatures.csv"
df_NOTstripped_f <- "data/Fig1B_EDFig1ABC/05_Fig1B_EDFig1ABC_NotStripped_CombinedFeatures.csv"
...
```{r}
# --- N per violin: Extended Data Figure 1B ---
# Groups: y = Marker (Oligo1/Oligo2/Blank/Unamplified), strip_status
cat("Extended Data Figure 1B - N per violin:\n")
df_cd11c_stripping %>% dplyr::count(y, strip_status) %>% print()
```
...
fig_cd11c_stripping <- plot_violin_comparison(
  ...
  xlim = c(100, NA),
  labs_title = "Extended Data Figure 1B - CD11c Stripping",
  axis = TRUE,
  axis_angle = 45,
  base_size = 10,
  dodge_width = 0.95,
  filename = "output/ExtendedDataForFig1/ExtendedDataFig1B.pdf",
  width_pt = 230,
  height_pt = 180
)
fig_cd11c_stripping
...
## Load dataframe
```{r}
df_edfig1d_f <- "data/EDFig1D/01_EDFig1D_linescans.csv"

df_edfig1d <- read_csv(df_edfig1d_f)
```

## Prepare data for plotting
```{r}
df_edfig1d_long <- df_edfig1d %>%
  pivot_longer(-pixel_distance,
               names_to  = c("image", "line"),
               names_sep = "_",
               values_to = "intensity")
```
...
## Plot
```{r}
edfig1d <- ggplot(df_edfig1d_long, aes(x = pixel_distance, y = intensity, colour = image)) +
  geom_line() +
  scale_colour_manual(values = c("con" = "cyan", "tyr" = "magenta")) +
  scale_linewidth(1) +
  facet_wrap(~line, nrow = 2, axes = "all") +
  labs(x = "Pixel Distance", y = "Intensity") +
  theme_minimal() +
  theme(
    ...
    text            = element_text(family = "Arial", size = 8, colour = "black")
  )


ggsave("output/ExtendedDataForFig1/ExtendedDataFig1D.csv", plot = edfig1d,
       width  = 170/72,
       height = 170/72,
       units  = "in",
       device = cairo_pdf)
...
## Load dataframes
```{r}
df_edfig2_f <- "data/EDFig2_3A/03_EDFig2_3A_RawFeaturesCellData.csv"

df_edfig2 <- read_csv(df_edfig2_f)
```

## Prepare data for plotting
```{r}
# Prepare data
df_negctrl <- df_edfig2 %>%
...
```{r}
# --- N per violin: Extended Data Figure 2 ---
# Groups: x = MarkerCond (Marker x condition interaction), Signal = PASTA/Unamp
cat("Extended Data Figure 2 Set 1 - N per violin:\n")
df_set1 %>% dplyr::count(x, Signal) %>% print()

cat("\nExtended Data Figure 2 Set 2 - N per violin:\n")
df_set2 %>% dplyr::count(x, Signal) %>% print()

cat("\nExtended Data Figure 2 Set 3 - N per violin:\n")
df_set3 %>% dplyr::count(x, Signal) %>% print()

cat("\nExtended Data Figure 2 Set 4 - N per violin:\n")
df_set4 %>% dplyr::count(x, Signal) %>% print()
```
...
fig_set1 <- plot_violin_vertical(
  ...
  ylim = c(0.01, 50000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig2_Set1.pdf",
  show_x_text = TRUE,
  width_pt = 130,
  height_pt = 135
)
...
fig_set2 <- plot_violin_vertical(
  ...
  fill_values = selected_colors,
  ylim = c(0.01, 50000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig2_Set2.pdf",
  show_x_text = TRUE,
  width_pt = 130,
  height_pt = 135
)
...
fig_set3 <- plot_violin_vertical(
  ...
  fill_values = selected_colors,
  ylim = c(0.01, 50000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig2_Set3.pdf",
  show_x_text = TRUE,
  width_pt = 130,
  height_pt = 135
)
...
fig_set4 <- plot_violin_vertical(
  ...
  fill_values = selected_colors,
  ylim = c(0.01, 50000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig2_Set4.pdf",
  show_x_text = TRUE,
  width_pt = 130,
  height_pt = 135
)
...
## Load data frame
```{r}
df_edfig3a_f <- "data/EDFig2_3A/03_EDFig2_3A_RawFeaturesCellData.csv"

df_edfig3a <- read_csv(df_edfig3a_f)
```

## Process data frame
```{r}
# Prepare data
df_fig3a_plot <- df_edfig3a %>%
  ...
  dplyr::mutate(
    ...
    Marker = dplyr::case_when(
      ...
      stringr::str_detect(RawMarkers, "ICOS") ~ "ICOS",
      stringr::str_detect(RawMarkers, "Pax5") ~ "Pax5"      
    )
  ) %>%
  replace_zeros_with_min(
    value_col = "Expression",
...
```{r}
# --- N per violin: Extended Data Fig 3A ---
# Groups: y = Marker, Signal = PASTA/Unamp
cat("Extended Data Fig 3A - N per violin:\n")
df_fig3a_plot %>% dplyr::count(y, Signal) %>% print()
```
...
```{r}
# Plot
edfig3a <- plot_violin_comparison(
  df_fig3a_plot,
  aes_fill = "Signal",
  fill_values = c(
    "Unamp" = "#e25726",
  ...
  labs_title = "Extended Data Figure 3A",
  axis = TRUE,
  axis_angle = 45,
  base_size = 6,
  dodge_width = 0.8,
  filename = "output/ExtendedDataForFig1/ExtendedDataFig3A.pdf",
  width_pt = 190,
  height_pt = 190
)
edfig3a
...
## Load dataframe
```{r}
df_edfig3b_f <- "data/EDFig3B/02_EDFig3B_RepDep_High_SCFeatures_ScaleSize.csv"

df_edfig3b <- read_csv(df_edfig3b_f)
```

## Prepare data for plotting
```{r}
# CD3 Markers
df_cd3_markers <- df_edfig3b %>%
...
```{r}
# --- N per violin: Extended Data Figure 3B ---
# No dodging (dodge_width = 0), so one violin per x level
cat("Extended Data Figure 3B CD3 markers - N per violin:\n")
df_cd3_markers %>% dplyr::count(x) %>% print()

cat("\nExtended Data Figure 3B CD8 markers - N per violin:\n")
df_cd8_markers %>% dplyr::count(x) %>% print()
```
...
fig_cd3_markers <- plot_violin_vertical(
  ...
  filename = "output/ExtendedDataForFig1/ExtendedDataFig3B_CD3.pdf",
  width_pt = 130,
  height_pt = 90
)
fig_cd3_markers
...
fig_cd8_markers <- plot_violin_vertical(
  ...
  base_size = 6,
  dodge_width = 0,
  filename = "output/ExtendedDataForFig1/ExtendedDataFig3B_CD8.pdf",
  width_pt = 130,
  height_pt = 90
)
fig_cd8_markers
...
## Load data frame
```{r}
df_edfig4a_f <- "data/EDFig4A/03_EDFig4A_median_combined.csv"

df_edfig4a <- read_csv(df_edfig4a_f)

```

## Process data
```{r}
df_edfig4a$condition <- factor(df_edfig4a$condition,
                              levels = c("thin", "medium", "thick"))

df_cd3_condition <- df_edfig4a %>%
  tidyr::pivot_longer(
...
```{r}
# --- N per violin: Extended Data Figure 4A ---
# Groups: x = condition (thin/medium/thick), Marker = fill column
cat("Extended Data Figure 4A CD3 - N per violin:\n")
df_cd3_condition %>% dplyr::count(x, Marker) %>% print()

cat("\nExtended Data Figure 4A CD8 - N per violin:\n")
df_cd8_condition %>% dplyr::count(x, Marker) %>% print()

cat("\nExtended Data Figure 4A CD11c - N per violin:\n")
df_cd11c_condition %>% dplyr::count(x, Marker) %>% print()

# CD69 has only one marker so just count by condition
cat("\nExtended Data Figure 4A CD69 - N per violin:\n")
df_cd69_condition %>% dplyr::count(x) %>% print()
```
...
fig_cd3_exp <- plot_violin_by_condition(
  df_cd3_condition,
  aes_fill = "Marker",
  fill_values = c("#e25726", RColorBrewer::brewer.pal(9, "Set1")[2]),
  ylim = c(300, 50000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig4A_CD3.pdf",
  show_x_text = TRUE,
  width_pt = 124,
  height_pt = 71
)
...
fig_cd8_exp <- plot_violin_by_condition(
  ...
  fill_values = c("#e25726", RColorBrewer::brewer.pal(9, "Set1")[2]),
  ylim = c(200, 30000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig4A_CD8.pdf",
  show_x_text = TRUE,
  width_pt = 124,
  height_pt = 71
)
...
fig_cd11c_exp <- plot_violin_by_condition(
  ...
  fill_values = c("#e25726", RColorBrewer::brewer.pal(9, "Set1")[2]),
  ylim = c(1000, 30000),
  filename = "output/ExtendedDataForFig1/ExtendedDataFig4A_CD11c.pdf",
  show_x_text = TRUE,
  width_pt = 124,
  height_pt = 71
)
...
fig_cd69_exp <- plot_violin_by_condition(
  ...
  filename = "output/ExtendedDataForFig1/ExtendedDataFig4A_CD69.pdf",
  show_x_text = TRUE,
  width_pt = 124,
  height_pt = 71
)
